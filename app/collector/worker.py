...
async def restore_history_buffer():
    """Відновлює останні 10 точок з бази даних для швидкого старту LSTM."""
    await send_system_log("🔄 Спроба відновити історію з БД для швидкого старту...", level="INFO", service="collector")
    try:
        ...
        if min_len > 0:
            ...
            await api_client.send_system_log("ℹ️ База порожня. Буфер почне заповнюватися з нуля.", "INFO", "collector")
            
    except Exception as e:
        await api_client.send_system_log(f"⚠️ Помилка відновлення буфера: {e}", "ERROR", "collector")
...
async def process_metrics_task(sys_settings: dict) -> None:
    ...
    try:
        ...
        for attempt in range(max_retries):
            ...
            for resource, query in queries.items():
                ...
                try:
                    ...
                    val = None
                    await send_system_log(f"⚠️ Помилка з'єднання з Prometheus: {e}", level="ERROR", service="collector")

                if val is None:
                    await send_system_log(f"   ⏳ Не вдалося отримати '{resource}'. Спроба {attempt + 1} з {max_retries}. Чекаємо {retry_delay} сек...", level="WARNING", service="collector")
        ...
        if not fetch_success:
            await send_system_log("❌ Prometheus недоступний. Пропускаємо цей цикл, щоб не псувати історію нулями.", level="ERROR", service="collector")
            return
        ...
        await send_system_log(
            f"   📥 Зібрано: CPU={point['cpu']:.2f}, RAM={point['ram']:.2f}, RPS={point['rps']:.2f}", level="INFO", service="collector"
        )
        ...
        if len(history_buffer) == 10:
            ...
            if predictions:
                await send_system_log(
                    f"   🔮 Прогноз: CPU={predictions['cpu']:.2f}, RAM={predictions['ram']:.2f}, RPS={predictions['rps']:.2f}", level="INFO", service="collector"
                )
            ...
            await send_system_log(
                f"   ⏳ Накопичення історії: {len(history_buffer)}/10. Прогноз пропускаємо.", level="INFO", service="collector"
            )

    except Exception as e:
        await send_system_log(f"❌ Непередбачена помилка у фоновій тасці: {e}", level="ERROR", service="collector")
    finally:
        is_busy = False


async def main() -> None:
    await send_system_log("🚀 Collector Service запущено у Динамічному режимі.", level="INFO", service="collector")

    # Запускаємо сервер метрик Prometheus на порту 8001
    start_http_server(8001, addr="0.0.0.0")
    await send_system_log("📡 Prometheus Exporter запущено на порту 8001 (/metrics)", level="INFO", service="collector")
    ...
    while True:
        try:
            ...
            await send_system_log(
                f"⚠️ Не вдалося отримати налаштування з API ({e}). Використовуються локальні дефолти.", level="WARNING", service="collector"
            )
        ...
        if is_active:
            ...
            await send_system_log(f"⏸️ Датаколектор вимкнено через Дашборд. Чекаємо {interval} сек...", level="INFO", service="collector")
