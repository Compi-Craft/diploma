...
async def run_finetune_pipeline(cmd: RetrainCommand):
    """Фонова задача: збирає дані, шляхи і запускає навчання."""
    try:
        async with aiohttp.ClientSession() as session:
            # 1. Дістаємо шляхи до моделі
            async with session.get(f"{API_URL}/models/byversion/{cmd.target_version}") as resp:
                if resp.status != 200:
                    await send_system_log(f"❌ Не вдалося знайти модель {cmd.target_version}", level="ERROR", service="lstm_module")
                    return
            ...
            async with session.get(url) as resp:
                if resp.status != 200:
                    await send_system_log(f"❌ Не вдалося завантажити дані для навчання", level="ERROR", service="lstm_module")
                    return
        ...
        for item in history_data:
            ...
            if last_ts is None or (current_ts - last_ts).total_seconds() > 2.0:
                ...
                if "cpu" in current_bucket and "ram" in current_bucket and "rps" in current_bucket:
                    raw_values.append([current_bucket["cpu"], current_bucket["ram"], current_bucket["rps"]])
        ...
        if len(raw_array) < 50:
            await send_system_log(f"❌ Замало повних даних для донавчання: {len(raw_array)} точок.", level="ERROR", service="lstm_module")
            return

        # 4. Передаємо все в ModelManager
        await send_system_log(f"🚀 Запуск Fine-Tuning для {cmd.target_version} на {len(raw_values)} точках...", level="INFO", service="lstm_module")
        ...
        await send_system_log(f"❌ Помилка у пайплайні донавчання: {e}", level="ERROR", service="lstm_module")
